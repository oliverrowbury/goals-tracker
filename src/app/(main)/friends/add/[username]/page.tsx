...
import { ownActivity, type OwnActivityEntry } from "@/lib/friends";
...
export default async function ProfilePage({ params }: { params: Promise<{ username: string }> }) {
  ...
  let journalStreak: number | null;
  let studyStreak: number | null;
  let workoutStreak: number | null;
  let activityEntries: ActivityEntry[] | OwnActivityEntry[];

  if (isSelf) {
    const own = await ownActivity(target.id, user.weightUnit, user.distanceUnit, today);
    journalStreak = own.journalStreak;
    studyStreak = own.studyStreak;
    workoutStreak = own.workoutStreak;
    activityEntries = own.entries;
  } else if (isFollowing) {
    // Only computed once there's an ACCEPTED follow — same privacy
    // boundary as the main Friends page, just for one person instead of a
    // whole list.
    ...
    const [profileWorkouts, profileStudySessions] = await Promise.all([
      target.shareWorkoutStreak
  ...
  const activityWorkoutIds = activityEntries.filter((e) => e.kind === "workout").map((e) => e.id);
  const activityStudySessionIds = activityEntries.filter((e) => e.kind === "study").map((e) => e.id);
