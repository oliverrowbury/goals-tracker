...
export default async function ProfilePage({ params }: { params: Promise<{ username: string }> }) {
  ...
  // Your own profile always shows your own activity — the shareXStreak
  // switches are about what a friend sees, not what you see looking at
  // yourself, so they're only consulted below when it isn't isSelf.
  const showActivity = isSelf || isFollowing;
  const today = todayISO();

  // Only computed once there's an ACCEPTED follow (or it's your own
  // profile) — same privacy boundary as the main Friends page, just for
  // one person instead of a whole list.
  const [journalDates, studyDates, workoutDates] = showActivity
    ? await Promise.all([
        isSelf || target.shareJournalStreak
          ? prisma.journalEntry.findMany({ where: { userId: target.id, bodyText: { not: "" } }, select: { date: true } })
          : null,
        isSelf || target.shareStudyStreak
          ? prisma.studySession.findMany({ where: { userId: target.id, durationMinutes: { not: null } }, select: { startedAt: true } })
          : null,
        isSelf || target.shareWorkoutStreak
  ...
  // archives it. Same privacy gate as the streaks above. Your own profile
  // shows every non-archived post regardless of visibility (PRIVATE ones
  // included) — that switch is about hiding a post from friends, not from
  // yourself.
  const ACTIVITY_LIMIT = 30;
  const [profileWorkouts, profileStudySessions] = showActivity
    ? await Promise.all([
        isSelf || target.shareWorkoutStreak
          ? prisma.workout.findMany({
              where: {
                ...
                endedAt: { not: null },
                archived: false,
                ...(isSelf ? {} : { visibility: "FRIENDS" as const }),
              },
  ...
  const activitySection = showActivity && hasAnyActivity && (
    <section className="mt-6">
      <h2 className="mb-3 text-sm font-medium text-ink-muted">Activity</h2>
      {profileActivity.length === 0 ? (
        <EmptyState message={isSelf ? "Nothing posted yet." : `Nothing from ${target.name} yet.`} />
      ) : (
  ...
  return (
    <div className="mx-auto max-w-sm">
      {isSelf ? (
        ...
        <>
          <div className="rounded-2xl border border-line bg-card p-6 shadow-sm">{cardContent}</div>
          {activitySection}
        </>
      )}
